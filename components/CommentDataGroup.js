...
const CommentDataGroup = ({ post, DocName }) => {
  ...
  console.log(DocName);
  useEffect(() => {
    const q = query(collection(FIRESTORE_DB, 'groupPost', DocName, 'posts', postId, 'comment'), orderBy('timestamp', 'desc'));
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      ...
      const commentRef = doc(FIRESTORE_DB, 'groupPost', DocName, 'posts', postId, 'comment',  commentId);
          ...
          <View style={styles.dropdown}>
            <TouchableOpacity onPress={() => handleDeleteComment(commentItem.id)}>
              ...
              style={{borderWidth:1, 
                ...
                onPress={() =>
                  navigation.navigate('ReplyGroup', {
...
export default CommentDataGroup; 